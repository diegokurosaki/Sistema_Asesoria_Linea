...
try {
    ...
    $sql = "WITH TotalHoras AS (
                ...
                WHERE 
                    C.Fch_Cita BETWEEN ? AND ?
                AND 
                    E.ID_Usuario_E = ?)
            SELECT 
                CASE
                    WHEN MONTH(C.Fch_Cita) = 1 THEN 'Enero'
                    WHEN MONTH(C.Fch_Cita) = 2 THEN 'Febrero'
                    WHEN MONTH(C.Fch_Cita) = 3 THEN 'Marzo'
                    WHEN MONTH(C.Fch_Cita) = 4 THEN 'Abril'
                    WHEN MONTH(C.Fch_Cita) = 5 THEN 'Mayo'
                    WHEN MONTH(C.Fch_Cita) = 6 THEN 'Junio'
                    WHEN MONTH(C.Fch_Cita) = 7 THEN 'Julio'
                    WHEN MONTH(C.Fch_Cita) = 8 THEN 'Agosto'
                    WHEN MONTH(C.Fch_Cita) = 9 THEN 'Septiembre'
                    WHEN MONTH(C.Fch_Cita) = 10 THEN 'Octubre'
                    WHEN MONTH(C.Fch_Cita) = 11 THEN 'Noviembre'
                    WHEN MONTH(C.Fch_Cita) = 12 THEN 'Diciembre'
                END AS Mes,
                COUNT(*) AS Total_Horas,
                ROUND((COUNT(*) / (SELECT TotalGlobal FROM TotalHoras)) * 100, 2) AS Porcentaje
            FROM 
                Citas C
            INNER JOIN 
                Estudiante E ON C.ID_Estudiante_Citado = E.ID_Usuario_E
    ...
    $stmt->bind_param('ssissi', $fecha_inicio, $fecha_fin, $IDEstudiante, $fecha_inicio, $fecha_fin, $IDEstudiante); // Protección contra inyección SQL
    $stmt->execute();
    ...
    if ($resultado->num_rows > 0) {
        ...
        $pdf->Cell(50, 7, "Total Horas", 1, 0, "C", true);
        ...
        while ($fila = $resultado->fetch_assoc()) {
            ...
            $pdf->Cell(50, 7, $fila['Total_Horas'], 1, 0, "C");
