...
// Consulta a la base de datos para obtener el top 5 de materiales mejor evaluados
$sql = "SELECT 
...
$data = array();
$data[] = ['Material', 'Calificación Promedio']; // Agregar la cabecera al array

// Agregar los datos de la consulta al array
while ($fila = $resultado->fetch_assoc()) {
    $data[] = ['Material ' . $fila['Titulo'], (float)$fila['Calificacion_Promedio']];
}
...
<head>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // Dibujar el gráfico
        function drawChart() {
            // Convertir datos PHP en objeto DataTable de Google Charts
            var data = google.visualization.arrayToDataTable(<?php echo $jsonData; ?>);

            var options = {
                title: 'Top 5 de Materiales Mejor Calificados',
                is3D: true,
                slices: {
                    0: {offset: 0.1},  // Resaltar el primer slice
                    1: {offset: 0.05}, // Resaltar el segundo slice
                    2: {offset: 0.05}, // Resaltar el tercer slice
                    3: {offset: 0.05}, // Resaltar el cuarto slice
                    4: {offset: 0.05}  // Resaltar el quinto slice
                }
            };

            // Dibujar el gráfico de pastel en el contenedor con ID 'grafico'
...
<div class="container mt-4">
    ...
    <div class="container mt-5">
        <form method="post" id="hacer_pdf" action="../../../Controlador/Reportes/Reporte_Mejor_Calificado.php" enctype="multipart/form-data"> 
