...
if (isset($_GET['id']) && !empty($_GET['id']) && isset($_GET['rol']) && !empty($_GET['rol'])) {
    ...
    $consulta = $conexion->prepare("SELECT E.Nombre_Usuario_E AS nombre_usuario, 
            E.Apellido_Materno_E AS apellido_materno, 
            E.Apellido_Paterno_E AS apellido_paterno, 
            E.Telefono_E AS telefono, 
            E.Fecha_Nacimiento_E AS fecha_nacimiento, 
            E.Genero_E AS genero, 
            E.Correo_electronico_E AS correo_electronico, 
            E.Contrasena_E AS contrasena, 
            E.IdCarrera 
            FROM 
                Estudiante E
            INNER JOIN 
    ...
    // La variable $_GET['id'] o $_GET['rol'] no está definida o es nula
    echo "ID de empleado o rol no proporcionado.";
    exit;
}
?>
        ...
        <input type="hidden" id="tipo_usuario" name="tipo_usuario" value="<?php echo $rol; ?>">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="ID_Carrera" name="ID_Carrera" required>
                    ...
                    <?php
                        $sql_tipos_carrera = $conexion->query("SELECT * FROM Carrera");
                        while($resultado_tipo_carrera = $sql_tipos_carrera->fetch_assoc()){
                            $selected = ($row['IdCarrera'] == $resultado_tipo_carrera['Id_Carrera']) ? 'selected' : '';
                            echo "<option value='".$resultado_tipo_carrera['Id_Carrera']."' $selected>".$resultado_tipo_carrera['Nombre_Carrera']."</option>";
                        }
                    ?>
                </select>
                <div class="invalid-feedback">
